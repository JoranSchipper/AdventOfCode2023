...
// list = ['bnjqlftwobvsvjqptdp1two94twonej'];

let listOfNumbers = list.map((listItem) => {
    let indexes = [
        {
            'key': '1',
            'matches': [...listItem.matchAll(/1/g)],
            'number': '1',
        },
        {
            'key': '2',
            'matches': [...listItem.matchAll(/2/g)],
            'number': '2',
        },
        {
            'key': '3',
            'matches': [...listItem.matchAll(/3/g)],
            'number': '3',
        },
        {
            'key': '4',
            'matches': [...listItem.matchAll(/4/g)],
            'number': '4',
        },
        {
            'key': '5',
            'matches': [...listItem.matchAll(/5/g)],
            'number': '5',
        },
        {
            'key': '6',
            'matches': [...listItem.matchAll(/6/g)],
            'number': '6',
        },
        {
            'key': '7',
            'matches': [...listItem.matchAll(/7/g)],
            'number': '7',
        },
        {
            'key': '8',
            'matches': [...listItem.matchAll(/8/g)],
            'number': '8',
        },
        {
            'key': '9',
            'matches': [...listItem.matchAll(/9/g)],
            'number': '9',
        },
        {
            'key': 'one',
            'matches': [...listItem.matchAll(/one/g)],
            'number': '1',
        },
        {
            'key': 'two',
            'matches': [...listItem.matchAll(/two/g)],
            'number': '2',
        },
        {
            'key': 'three',
            'matches': [...listItem.matchAll(/three/g)],
            'number': '3',
        },
        {
            'key': 'four',
            'matches': [...listItem.matchAll(/four/g)],
            'number': '4',
        },
        {
            'key': 'five',
            'matches': [...listItem.matchAll(/five/g)],
            'number': '5',
        },
        {
            'key': 'six',
            'matches': [...listItem.matchAll(/six/g)],
            'number': '6',
        },
        {
            'key': 'seven',
            'matches': [...listItem.matchAll(/seven/g)],
            'number': '7',
        },
        {
            'key': 'eight',
            'matches': [...listItem.matchAll(/eight/g)],
            'number': '8',
        },
        {
            'key': 'nine',
            'matches': [...listItem.matchAll(/nine/g)],
            'number': '9',
        }
    ];

    let filteredIndexes = indexes.filter((index) => index.matches.length > 0);
    let allIndexes = filteredIndexes.map((index) => index.matches.map(({match, index}) => index));

    let lowestIndex = Math.min(...allIndexes.flat());
    let highestIndex = Math.max(...allIndexes.flat());

    let lowestIndexNumber = filteredIndexes.find((index) => index.matches.map(({match, index}) => index).includes(lowestIndex)).number;
    let highestIndexNumber = filteredIndexes.find((index) => index.matches.map(({match, index}) => index).includes(highestIndex)).number;

    return `${lowestIndexNumber}${highestIndexNumber}`;
});

let sum = listOfNumbers.reduce((accumulator, currentValue) => Number(accumulator) + Number(currentValue), 0);
console.log(sum);